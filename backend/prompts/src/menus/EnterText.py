...
class SafeFormatter(dict):
    """A dictionary subclass for safe string formatting (returns the key with braces if missing)"""
...
class QuestionCreatePromptTemplate:
    ...
    def create_prompttemplate(self, input_data: dict) -> ChatPromptTemplate:
        ...
        """
        try:
            # Create a safe copy of the input data with defaults
            safe_input = SafeFormatter(input_data)
            ...
            raw_option_type_value = str(input_data.get("optionTypeValue", "alphabetical")).strip()
            ...
            system_template = (
                "You are an expert educational question generator. Your goal is to generate high-quality questions based on the provided text.\n"
                "Strictly follow these rules:\n"
                "1. Generate exactly {numQuestionsValue} questions.\n"
                "2. ALL questions must be of type: '{questionType}'. Do NOT mix question types.\n"
            ...
            answer_formats = {
                "MCQ": f"   Correct Answer: [Correct option label and full text, e.g., 'b) {sample_answer_format}...']\n",
            ...
            type_specific = {
                "MCQ": (
                    ...
                    "IMPORTANT: The 'options' array MUST include the labels (e.g., 'A. ', '1. ', 'a) ') as the first characters of each option string, matching the chosen '{optionTypeValue}' style exactly.\n"
                    "Do NOT omit labels from the options array.\n"
                    f"{answer_section}"
                ),
                "Fill in the blanks": (
                ...
                "Match the following": (
                    ...
                    "PERFECT FORMAT:\n"
                    "Match the following concepts from the passage:\n"
                    "   Column A\t\tColumn B\n"
                    "   a. [Item 1]\t\ti. [Match 1]\n"
                    "   b. [Item 2]\t\tii. [Match 2]\n"
                    "   ...\n"
                ...
                "True/False": (
                    "FORMAT EXAMPLE:\n"
                    "[Statement based on passage]. (True/False)\n"
                    "{answer_section}"
                ),
                ...
                "Essay Questions": (
                    "FORMAT EXAMPLE:\n"
                    "[In-depth question about concepts in the passage].\n"
                    "   Expected length: {essayLengthValue}\n"
                    "{answer_section}"
                ),
                "Numerical Problems": (
                    "FORMAT EXAMPLE:\n"
                    "[Mathematical problem based on numerical information in the passage]\n"
                    "{answer_section}"
                ),
                "Programming Exercise": (
                    "FORMAT EXAMPLE:\n"
                    "[Programming task related to concepts in the passage]\n"
                    "{answer_section}"
                ),
                "Statement Problems": (
                    "FORMAT EXAMPLE:\n"
                    "Consider the following statements based on the passage:\n"
                    "   I. [Statement 1]\n"
                    "   II. [Statement 2]\n"
                    "   A) Only I\n"
                    "   B) Only II\n"
            ...
            type_req = type_specific.get(
                ...
                SafeFormatter({
                    ...
                    "numberOfMissingWordsValue": input_data.get("numberOfMissingWordsValue", "1"),
            ...
            text_to_generate_question = (
                ...
                "{text}\n"
                "--- SOURCE TEXT END ---\n\n"
                "GOAL: Generate {numQuestionsValue} educational questions of type '{questionType}' based ONLY on the source text above.\n\n"
                "SPECIFICATIONS:\n"
            ...
            # Define the output requirements - strictly enforced JSON structure
            json_structure_instruction = (
                ...
                + ('      "options": ["A. Option 1", "B. Option 2", "C. Option 3", "D. Option 4"],\n' if question_type in ['MCQ', 'Statement Problems'] else "")
                + '      "answer": "Correct answer label and full text (e.g., \'b) Option text\')"'
                + (f',\n      "explanation": "Brief explanation"' if explanation_detail != "Not required" else "")
                + "\n    }\n"
                "  ]\n"
                "}\n"
                "DO NOT include the source text in your JSON output. ONLY output the questions."
            )
            ...
            chat_template = ChatPromptTemplate.from_messages(
                [
                    ...
                    HumanMessage(content=
                        text_to_generate_question.format_map(
                            SafeFormatter({
                                "type_specific_requirements": type_req,
                                **safe_input
                            })
                        )
                        + final_instruction
            ...
            import traceback
            error_msg = f"Prompt creation error: {str(e)}\n{traceback.format_exc()}"
            print(error_msg)
            # Try to log to a file as well if possible
            try:
                with open("prompt_error.log", "a") as f:
                    f.write(error_msg + "\n")
            except:
                pass
            return None